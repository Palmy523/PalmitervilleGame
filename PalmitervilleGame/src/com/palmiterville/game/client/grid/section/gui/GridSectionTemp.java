...
public class GridSectionTemp extends SimplePanel implements Comparable<GridSectionTemp> {
	...
	private static GridSectionTemp selectedSection = null;
	...
	public GridSectionTemp(Coordinates coordinates) {
		super();
	...
	public GridSectionTemp(int rowIndex, int columnIndex) {
		this(new Coordinates(rowIndex, columnIndex));
	}
	...
	public void select(SelectionType type) {
		...
		if (type == SelectionType.NONE) {
			if (this.isOccupied) {
				...
				if (item instanceof HasTurn) {
					...
					if (hasTurn.isTurn()) {
						this.setStyleName(GridSectionTemp.SELECTED_HASTURN_STYLE);
		...
		this.setStyleName(GridSectionTemp.SELECTED_STYLE);
		selectedSection = this;
	}
	...
	public void deselect() {
		...
		if (this.isOccupied) {
			...
			if (item instanceof HasTurn) {
				HasTurn hasTurn = (HasTurn) item;
				if (hasTurn.isTurn()) {
					this.setStyleName(GridSectionTemp.HASTURN_STYLE);
					return;
				}
			}
		} 
	...
	@Override
	public int compareTo(GridSectionTemp section) {
	...
	public boolean equals(Object obj) {
		...
		if (obj == null)
			return false;
		if (!(obj instanceof GridSectionTemp))
			return false;
